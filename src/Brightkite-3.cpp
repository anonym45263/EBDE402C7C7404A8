...
int main() {
    ...
    out.open("Brightkite-1-" + to_string(B) + ".txt");
    for (int O = 4096; O <= 2097152; O *= 2){
        N = O;
        cnt = qcnt = 0;
        Tree *tree = BuildTree();
        RandomQuery(10000, tree, 0.05);
        cout << O << "\t" << cnt / 10000.0 << "\t" << qcnt / 10000.0 * log(O) / log(B) << endl;
        out << O << "\t" << cnt / 10000.0 << "\t" << qcnt / 10000.0 * log(O) / log(B) << endl;
        delete tree;
    }
