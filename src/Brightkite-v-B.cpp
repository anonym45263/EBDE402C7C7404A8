...
void PrintMeasurements(int scale, int max_print=10) {
    ...
    auto printRow = [&](const string& name, double values[], int begin, int scale, bool print_b = false, int b_list[] = nullptr) {
        cout << left << setw(NAME_WIDTH) << name;
        for (int i = begin; i < scale; i++) {
            if (print_b) {
                cout << right << setw(VALUE_WIDTH) << b_list[i];
            } else {
    ...
    for (int i = begin; i < scale; i++) { b_list[i] = 1 << (i + 2); }
    printRow("B", values, begin, scale, true, b_list);
...
void BuildTree() {
    ...
    int l = 0;
    int node_cnt = N / B;
    // cout << "# leaves created: " << node_cnt << endl;
    while (node_cnt > l + 1) {
        // cout << "Internal node creation loop start from l = " << l << ", node_cnt = " << node_cnt << endl;
        int r = node_cnt;
        ...
        // cout << "# nodes created: " << (node_cnt - r) << endl;
        if (r - l < B) {
            ...
            node_cnt++;
            // cout << "Just now no nodes created, meaning possibly r - l < B, which is the root case, and thus created r - l nodes, and B - (r - l) dummies" << endl;
        }
        l = r;
    }
...
int main() {
    ...
    ofstream ofs("plot/Brightkite-v-B.sta");
    ...
    for (B = 4; B <= 128; B *= 2) {
        ...
        cout << "Build classical tree..." << endl;
        BuildTree();
        cout << "Done" << endl;
        cout << "Build quantum tree..." << endl;
        BuildTreeQuantum();
        cout << "Done" << endl;
