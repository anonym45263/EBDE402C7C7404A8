...
// void QuantumQuery_old(double x, double y, Tree *tree, vector<int> V, int k) {
//     vector<int> U;
...
//         for (int i = 0; i < B; i++) {
//             meas[meas_i].c_query_qu_accum++;
//             if ((x <= tree->node[V[j]].l[i]) && (tree->node[V[j]].l[i] <= y)) {
//                 // the current child range [l, r] is completely within the query range [x, y]
//                 meas[meas_i].c_query_qu_accum += QuantumSim(height, weight, k);
//                 return;
//             } else if ((x <= tree->node[V[j]].r[i]) && (tree->node[V[j]].l[i] <= y) && (tree->node[V[j]].c[i] != -1)) {
//                 // the current child range [l, r] has overlap with the query range[x, y] and the child is not dummy
...
void RandomQuery(int repeat, double rate) {
    ...
    for (int i = 0; i < repeat; i++) {
        ...
        // vector<int> V;
        // V.push_back(tree->root);
        // QuantumQuery_old(x, y, tree, V, k);
        QuantumQuery(x, y, k);

        // QuantumQuery(1209807281, 1209811499);
        // QuantumQuery(1210284323, 1215527708);
        // QuantumQuery(1215888627, 1215898827);
        // QuantumQuery(1215888627, 1216336185);
    }
}
...
int main() {
    ...
    ofstream ofs("plot/Brightkite-v-N.sta");
    ...
    for (N = (1 << 12); N <= (1 << 27); N *= 2) {
        ...
        // cout << "tree->node[0]: [" << tree->node[0].l[0] << ", " << tree->node[0].r[B - 1] << "]" << endl;
        // cout << "tree->node[1]: [" << tree->node[1].l[0] << ", " << tree->node[1].r[B - 1] << "]" << endl;
        // cout << "tree->node[15]: [" << tree->node[15].l[0] << ", " << tree->node[15].r[B - 1] << "]" << endl;
        // cout << "tree->node[16]: [" << tree->node[16].l[0] << ", " << tree->node[16].r[B - 1] << "]" << endl;
        // cout << "tree->node[17]: [" << tree->node[17].l[0] << ", " << tree->node[17].r[B - 1] << "]" << endl;
