...
const vector<int> range_N = {1 << 12, 1 << 13, 1 << 14, 1 << 15, 1 << 16, 1 << 17, 1 << 18, 1 << 19, 1 << 20};
const int B = 8;
...
int LinearScan(const string& query_title, const vector<MovieRecord*>& result) {
    int cost = 0;
    for (int i = 0; i < result.size(); i++) {
        cost++;
        if (result[i]->primaryTitle == query_title) {
            // cout << "-> Answer: " << result[i]->tconst << ", " << result[i]->primaryTitle << ", " << result[i]->releaseTimestamp << endl;
            return cost;
        }
    }
...
void run_cl(const Tree* tree, double x, double y, string query_title) {
    ...
    int cost_linear_scan = LinearScan(query_title, result);
...
void run_hy(const Tree* tree, double x, double y, string query_title) {
    vector<MovieRecord*> result;
    auto start = chrono::high_resolution_clock::now();
    ...
    // now, we will simulate using the classical result for quantum search
...
// void run_hy2(const Tree* tree, double x, double y, string query_title) {
//     vector<MovieRecord*> result;
...
void run_qu(const Tree* tree, double x, double y, string query_title) {
    vector<MovieRecord*> result;
    auto quantum_query_result = QuantumQuery(tree, x, y, result);
...
void RandomQuery(const Tree* tree, const pair<double, MovieRecord*>* input, int N, int repeat, double rate) {
    ...
    for (int i = 0; i < repeat; i++) {
        ...
        string query_title = "";
        if (Random(100) % 2 == 0) {
            int random_n = Random(n - 10);
            query_title = input_copy[t + random_n].second->primaryTitle;
        } else {
            int random_N = Random(N);
            query_title = input[random_N].second->primaryTitle;
        }

        run_cl(tree, x, y, query_title);
        run_hy(tree, x, y, query_title);
        run_qu(tree, x, y, query_title);
...
int main() {
    ...
    ofstream ofs("plot/IMDb_search-v-N.dat");
