...
std::pair<int, double> performHashJoin(const double x, const double y, const std::vector<Orders>& orders, const std::vector<LineItem>& lineitems) {
    auto start = std::chrono::high_resolution_clock::now();
    int cost = 0;
    // find the orders with totalprice between x and y
    std::vector<Orders> orders_in_range;
    ...
    // perform a hash join between orders and lineitems, where order.totalprice is between x and y
    ...
    std::unordered_map<std::string, Orders> orders_hash_table;
    orders_hash_table.clear();
    for (const auto& order : orders_in_range) {
        orders_hash_table[computeHash(std::to_string(order.orderkey))] = order;
        cost++;
    }
    // std::cout << "Number of orders in orders_hash_table: " << orders_hash_table.size() << std::endl;
    // for (const auto& order_hash : orders_hash_table) {
    //     std::cout << "Order hash key: " << order_hash.first << " with orderkey " << order_hash.second.orderkey << std::endl;
    // }

    // perform a hash join between orders_hash_table and lineitems
    for (const auto& lineitem : lineitems) {
        cost++;
        if (orders_hash_table.find(computeHash(std::to_string(lineitem.orderkey))) != orders_hash_table.end()) {
            results.push_back(lineitem);
...
void RandomQuery(const std::string& alg, const Tree* tree, double x, double y, int N, double rate) {
    ...
    if (alg == "cl") {
        ...
        estimation = int(rate * double(N));
    }
    // std::cout << "Number of orders in result: " << estimation << std::endl;
    ...
    if (estimation <= 55) {
        res = performNestLoopJoin(x, y, orders, lineitems);
    } else {
        res = performHashJoin(x, y, orders, lineitems);
    }

    if (alg == "cl") {
        meas[meas_i].c_query_cl_accum += double(res.first);
        meas[meas_i].t_query_cl_accum += res.second;
        meas[meas_i].c_query_cl_pr_accum += double(res.first);
        meas[meas_i].t_query_cl_pr_accum += res.second;
    } else if (alg == "qu") {
        meas[meas_i].c_query_qu_accum += double(res.first);
        meas[meas_i].t_query_qu_accum += res.second;
        meas[meas_i].c_query_qu_pr_accum += double(res.first);
        meas[meas_i].t_query_qu_pr_accum += res.second;
    } else if (alg == "es") {
        meas[meas_i].c_query_es_accum += double(res.first);
        meas[meas_i].t_query_es_accum += res.second;
        meas[meas_i].c_query_es_pr_accum += double(res.first);
        meas[meas_i].t_query_es_pr_accum += res.second;
    }
}
...
int main() {
    ...
    try {
        std::ofstream ofs("plot/TPCH_join1.dat");
