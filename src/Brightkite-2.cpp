    ...
    while (!V.empty()){
        ...
        for (int i = 0; i < B; i++) {
            if ((tree->node[u].l[i] >= x) && (tree->node[u].l[i] <= y)) {
                ...
                qcnt += QuantumSim(tree->node[u].height + V.size(), weight, k);
                return;
...
void RandomQuery(int I, Tree * tree, double rate){
    for (int i = 0; i < N; i++)
        tmp[i] = input[i];
    quicksort(tmp, 0, N - 1);
    int n = N * rate;
    for (int i = 0; i < I; i++){
        int t = Random(N - n - 1);
        double x1 = tmp[t].first;
        double y1 = tmp[t + n].first;
        double x2 = 1e9;
        double y2 = -1e9;
        for (int j = t; j <= t + n; j++){
            x2 = min(x2, tmp[j].second);
            y2 = max(y2, tmp[j].second);
        }
        V.clear();
...
int main() {
    ...
    ReadInput("brightkite_totalCheckins.txt");
    out.open("Brightkite-2-" + to_string(B) + ".txt");
    for (double O = 0.05; O <= 0.05; O += 0.01){
        cnt = qcnt = 0;
        cntans = 0;
        Tree *tree = BuildTree();
        RandomQuery(10000, tree, O);
